(lambda (env)
  ...
  ;; identical requests.
  ;;
  ;; I failed to design the sack api to do the above paragraph.
  ...
  ;; The list '(0 2), representing this version of Sack.
  (env 'sack:version)
  ...
  ((env 'sack:cookie:get-all)) ;; Returns a list of the cookies,
                               ...
                               ;; app. TODO Why does this return a
                               ;; procedure
  ((env 'sack:cookie:get) name) ;; Returns a cookie or #f
  ((env 'sack:cookie:set!) cookie)

  ((env 'sack:session:get-all)) ;; Returns an a-list of the session
                                ;; data. TODO Why does this return a
                                ;; procedure?
  ((env 'sack:session:get) name)
  ((env 'sack:session:set!) name value) ;; Name must be a string
  ((env 'sack:session:delete!) name)
  ((env 'sack:session:destroy!)) ;; Destroy the session. TODO Why does
                                 ;; this return a procedure?
  ((env 'sack:session:get-id)) ;; Possibly not there. TODO Why does
                               ;; this return a procedure
  ((env 'sack:session:regenerate-id!)) ;; Possibly not there TODO Why
                                       ;; does this return a procedure
